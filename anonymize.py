...
@click.option(
    ...
    "--inputfolder",
    default=".",
    help="Input folder where dicoms to anonymize are present",
)
@click.option(
    "-f", "--force", is_flag=True, help="Force to remove output folder if present"
)
    ...
    for root, dirs, files in os.walk(".", topdown=True):
        ...
        for file in files:
            ...
            try:
                ...
                ):  # If Patient ID is present
                    realPatientId = str(encryptId(int(ds[(0x10, 0x20)].value)))
                anonymizeDicomFile(
                    os.path.join(root, file),
                    os.path.join(outputPath, root, file),
                    patientname,
                    realPatientId,
                    removedate,
                    tag,
                ...
                print(e)
                if (
                    not file.endswith(".dat")
                    and not file.endswith(".mhd")
                    and not file.endswith(".raw")
                    and not file.endswith(".INI")
                    and not file.endswith(".XVI")
                    and not file.endswith(".SCAN")
                    and not file.endswith(".REFSCAN")
                    and not file.endswith(".REFPATIENTORIENTATION")
                    and not file.endswith(".REFORIENTATION")
                    and not file.endswith(".DELINEATION")
                    and not file.endswith(".tar.bz2")
                    and not file.startswith("Angle.")
                    and not file.endswith(".jpg")
                    and not file.endswith(".his")
                ):
                    print(
                        os.path.join(inputfolder, root, file)
                        + " is not a correct dicom file"
                    )
                shutil.copyfile(
                    os.path.join(root, file), os.path.join(outputPath, root, file)
                )
    ...
    def test_anonymize(self):
        ...
        with open(
            os.path.join(
                tmpdirpath,
                "anonymizationOutput",
                "1.2.840.113619.2.281.3562.19216820010.1547461877.50454600.dcm",
            ),
            "rb",
        ) as fnew:
            bytesNew = fnew.read()
